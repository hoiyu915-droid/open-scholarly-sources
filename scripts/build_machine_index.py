...
def llms_text(manifest, records, base_url):
    ...
    lines = [
        ...
        f"- [Registry-brokered chatbot entry]({base_url}/chatbot-entry.txt): minimal no-Skill/no-server bootstrap using a GitHub-connector-pinned immutable release.",
        f"- [Registry-brokered routing JSON]({base_url}/chatbot-search-routing.json): dynamic registry-derived domain broker and optional direct-adapter contract.",
        f"- [Registry-brokered protocol]({base_url}/chatbot-search-protocol.md): original-source identity and evidence-scope OA search procedure.",
        "",
